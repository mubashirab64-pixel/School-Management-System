...
import { UploadCloud, CheckCircle2, XCircle, FileSpreadsheet, Download, Loader2, RefreshCw, Info, Eye, Users } from 'lucide-react'
...
export default function TeacherBulkUpload() {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/teachers/bulk-upload-template/`, {
      ...
      a.href = url
      a.download = 'teacher_bulk_upload_template.xls'
      a.click()
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/teachers/bulk-upload/`, {
        method: 'POST',
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        body: formData,
      })
      ...
      <Card className="border-none shadow-xl bg-gradient-to-r from-[#013a63] via-[#01497c] to-[#012a4a] text-white overflow-hidden relative">
        ...
        <CardHeader className="relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-3xl font-bold tracking-tight">Bulk Teacher Upload</CardTitle>
              <CardDescription className="text-blue-50 mt-1 text-base">
                Download the Excel template, populate it with teacher records, and seamlessly upload them back into the system.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          <CardContent className="space-y-6">
            <div className="bg-[#f0f4f8] rounded-xl p-5 border border-blue-100">
              ...
              <div className="flex flex-wrap gap-2 mb-5">
                {['full_name', 'gender', 'dob', 'contact_number', 'email', 'cnic', 'joining_date', 'campus', 'shift'].map(col => (
              ...
              <div className="flex flex-wrap gap-2">
                {['father_name', 'current_role_title', 'education_level', 'total_experience_years', 'marital_status', 'current_address'].map(col => (
              ...
              <div className="mt-5 pt-4 border-t border-gray-200/60">
                <p className="text-xs text-gray-500 leading-relaxed font-mono">
                  <strong className="text-gray-700">Formats:</strong> Gender: male/female | Dates: YYYY-MM-DD | Shift: morning/afternoon/both
                </p>
              </div>
            </div>
        ...
        <Card className="border-none shadow-lg bg-white/80 backdrop-blur-sm transition-all duration-300 hover:shadow-xl">
          <CardHeader>
            ...
            <CardDescription className="text-gray-500">
              Upload your completed spreadsheet. Valid teacher records will be created automatically.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {!reports ? (
              <div className="space-y-6">
                ...
                >
                  ...
                  {file ? (
                    ...
                    <div className="flex flex-col items-center gap-4 text-gray-400 group-hover:text-[#3b82f6] transition-colors">
                      ...
                      <div>
                        ...
                        <p className="text-sm mt-1 text-gray-500 group-hover:text-[#013a63]">Supports .csv</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {loading && (
                  <div className="space-y-2 animate-in fade-in">
                    ...
                    <div className="h-3 bg-gray-100 rounded-full overflow-hidden shadow-inner">
                      <div
                        className="h-full bg-gradient-to-r from-[#6096ba] to-[#013a63] transition-all duration-300 ease-out relative"
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        <UploadCloud className="w-5 h-5" /> Upload & Create
                      </>
                    )}
                  </Button>
              ...
              <div className="flex flex-col items-center justify-center h-full py-8 text-center space-y-4 animate-in zoom-in duration-500">
                ...
                <p className="text-gray-500 max-w-[250px]">
                  Your teacher records have been successfully processed. Check the detailed report below.
                </p>
                <Button 
                  onClick={handleReset}
                  variant="outline"
            ...
            <table className="w-full text-sm">
              <thead className="bg-[#f8fbfd] text-[#013a63] text-xs uppercase font-bold border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left tracking-wider">Teacher Name</th>
      ...
      <Dialog open={showGuide} onOpenChange={setShowGuide}>
        <DialogContent className="max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Details of mandatory fields for bulk teacher upload.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
              <h4 className="font-bold text-[#013a63] text-lg mb-1">full_name</h4>
              <p className="text-gray-600 text-sm">Full legal name of the teacher.</p>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
              <h4 className="font-bold text-[#013a63] text-lg mb-1">campus</h4>
              <p className="text-gray-600 text-sm">The exact name of the campus as registered in the system.</p>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
              <h4 className="font-bold text-[#013a63] text-lg mb-1">email & contact_number</h4>
              <p className="text-gray-600 text-sm">Used for account creation and official communications.</p>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
              <h4 className="font-bold text-[#013a63] text-lg mb-1">shift</h4>
              <p className="text-gray-600 text-sm">Must be "morning", "afternoon", or "both".</p>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
              <h4 className="font-bold text-[#013a63] text-lg mb-1">joining_date</h4>
              <p className="text-gray-600 text-sm">Format: YYYY-MM-DD. Critical for seniority and payroll.</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
