...
const HOUSE_COLORS: Record<string, string> = {
  "owned":         "#163B5C", // Dark navy — stable/strong
  "rented":        "#2F6B8A", // Medium blue
  "not specified": "#94a3b8", // Slate gray
  "not_specified": "#94a3b8",
}

const DEFAULT_COLORS = ["#163B5C", "#2F6B8A", "#E6AD45", "#E87A5D", "#10b981", "#94a3b8"]
...
export function HouseOwnershipChart({ data, isLoading }: HouseOwnershipChartProps) {
    ...
    return (
      <Card className="h-[400px] border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl flex flex-col bg-white">
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-bold text-[#163B5C]">House Ownership</CardTitle>
        </CardHeader>
  ...
  const processedData = data.map((item, i) => ({
    ...item,
    fill: HOUSE_COLORS[item.name.toLowerCase()] ?? DEFAULT_COLORS[i % DEFAULT_COLORS.length],
  }))
      ...
      return (
        <div className="bg-white border border-gray-100 rounded-xl p-3 shadow-xl text-xs">
          ...
          <p className="text-gray-500 mt-0.5">
            Families: <span className="font-semibold text-[#2F6B8A]">{item.value}</span>
          </p>
    ...
    <Card className="h-[400px] border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-shadow duration-300 rounded-2xl flex flex-col bg-white overflow-hidden">
      <CardHeader className="pb-2 bg-white flex flex-row items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            <CardTitle className="text-base font-bold text-[#163B5C]">House Ownership</CardTitle>
            <CardDescription className="text-xs text-gray-400">Family housing status</CardDescription>
          </div>
        </div>
      </CardHeader>
        ...
        {data.length === 0 ? (
          ...
          <div className="h-[200px] w-full relative">
            ...
            <div className="absolute top-[50%] left-[50%] transform -translate-x-1/2 -translate-y-[62%] text-center pointer-events-none">
              ...
              <div className="text-[9px] text-gray-400 font-semibold uppercase tracking-wider">Families</div>
            </div>
          </div>
        )}
      </CardContent>
